...
contract TheRewarderChallenge is Test {
    ...
    checkSolvedByPlayer
    {
        //   player:  0x44E97aF4418b7a17AABD8090bEA0A471a366305C position #188
        //   alice:  0x328809Bc894f92807417D2dAD6b7C998c1aFdac6
        bytes32[] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
        bytes32[] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");

        uint256 numberOfDvtTokens = (TOTAL_DVT_DISTRIBUTION_AMOUNT - ALICE_DVT_CLAIM_AMOUNT) / PLAYER_DVT_CLAIM_AMOUNT;
        // numberOfDvtTokens = 867
        uint256 numberOfWethTokens =
            (TOTAL_WETH_DISTRIBUTION_AMOUNT - ALICE_WETH_CLAIM_AMOUNT) / PLAYER_WETH_CLAIM_AMOUNT;
        // numberOfDvtTokens = 853

        IERC20[] memory tokensToClaim = new IERC20[](numberOfDvtTokens + numberOfWethTokens);
        Claim[] memory claims = new Claim[](numberOfDvtTokens + numberOfWethTokens);

        for (uint256 i = 0; i < numberOfDvtTokens; i++) {
        ...
        for (uint256 i = numberOfDvtTokens; i < numberOfDvtTokens + numberOfWethTokens; i++) {
        ...
        assertEq(dvt.balanceOf(player), PLAYER_DVT_CLAIM_AMOUNT * numberOfDvtTokens);
        assertEq(weth.balanceOf(player), PLAYER_WETH_CLAIM_AMOUNT * numberOfWethTokens);
